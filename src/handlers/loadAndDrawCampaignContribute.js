  ...
  getCampaigns({
    ...
    el('#campaign-contribute-to-campaign').addEventListener('click', () => {
      ...
      if (getAccountBalance().gte(web3.toWei('1', 'finney'))) {
        getRouter()(`/campaign/${campaignID}/contribute/verify`);
        history.pushState({}, null, `/campaign/${campaignID}/contribute/verify`);
      }
    });

    // contirbute to campaign buton
    el('#campaign-contribute-to-campaign-verify').addEventListener('click', () => {
      getRouter()(`/campaign/${campaignID}/contribute/verify`);
    ...
    setInterval(() => {
      ...
      if (currentPath.indexOf('/verify') !== -1) {
        currentStep = 1;
      }
      ...
      if (el('#contributeStep0')) {
        el('#contributeStep0').className = `col-xs-2 col-sm-2 bs-wizard-step
          ${currentStep >= 0 && 'complete' || 'disabled'}
          ${currentStep === 0 && 'current' || ''}`;
        el('#contributeStep1').className = `col-xs-2 col-sm-2 bs-wizard-step
          ${currentStep >= 1 && 'complete' || 'disabled'}
          ${currentStep === 1 && 'current' || ''}`;
        el('#contributeStep2').className = `col-xs-2 col-sm-2 bs-wizard-step
          ${currentStep >= 2 && 'complete' || 'disabled'}
          ${currentStep === 2 && 'current' || ''}`;
        el('#contributeStep3').className = `col-xs-2 col-sm-2 bs-wizard-step
          ${currentStep >= 3 && 'complete' || 'disabled'}
          ${currentStep === 3 && 'current' || ''}`;
        el('#contributeStep4').className = `col-xs-2 col-sm-2 bs-wizard-step
          ${currentStep >= 4 && 'complete' || 'disabled'}
